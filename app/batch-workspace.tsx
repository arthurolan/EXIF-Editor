...
export function BatchWorkspace({ language, onClose }: { language: Language; onClose: () => void }) {
  ...
  const addFiles = (files: FileList | null) => {
    ...
    if (accepted.length !== candidates.length) setSelectionError(t.selectionError);
  ...
  return <section className="batch-shell" aria-busy={running}>
    ...
    {selectionError && <p className="batch-error"><AlertTriangle size={16} />{selectionError}</p>}
    ...
    <div className="batch-list" aria-live="polite">
      {items.length === 0 ? <div className="batch-empty"><FileImage size={24} />{t.noFiles}</div> : items.map((item) => <article className={`batch-row status-${item.status}`} key={item.id}>
        ...
        {item.status === "success" && item.output ? <button type="button" className="text-button" onClick={() => { if (item.output) download(item.output); }}><Download size={16} />{t.download}</button> : item.status === "reading" || item.status === "writing" || item.status === "verifying" ? <RefreshCw className="spin" size={17} /> : item.status === "failed" ? <AlertTriangle size={17} /> : item.status === "success" ? <Check size={17} /> : null}
      </article>)}</div>
    <div className="batch-footer">
      <p>{t.noDelivery}</p>
      <div>
        {counts.success > 0 && <button type="button" className="secondary-button" onClick={() => items.forEach((item) => { if (item.output) download(item.output); })}><Download size={16} />{t.downloadAll}</button>}
