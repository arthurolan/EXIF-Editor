...
export function BatchWorkspace({ language, onClose }: { language: Language; onClose: () => void }) {
  ...
  const addFiles = (files: FileList | null) => {
    ...
    for (const source of candidates) {
      ...
      accepted.push({ id: `${source.name}-${source.size}-${source.lastModified}-${crypto.randomUUID()}`, source, operation, textEdits: { ...textEdits }, status: "waiting" });
    }
  ...
  const run = async (onlyFailed = false) => {
    ...
    for (const item of queue) {
      ...
      const result = await processBatchFile(item.source, item.operation, item.textEdits, (phase) => update(item.id, { status: phase }));
  ...
  return <section className="batch-shell" aria-busy={running}>
    ...
    <div className="batch-controls">
      <div className="batch-operation" role="radiogroup" aria-label={t.title}>
        ...
        <button type="button" role="radio" aria-checked={operation === "metadata"} className={operation === "metadata" ? "active" : ""} disabled={running} onClick={() => setOperation("metadata")}><PencilLine size={15} />{t.metadata}</button>
      </div>
      {operation === "metadata" && <section className="batch-metadata" aria-labelledby="batch-metadata-title">
        ...
        <div className="batch-metadata-fields">
          <label><span>{t.artist}</span><input value={textEdits.artist} onChange={(event) => assignText("artist", event.target.value)} placeholder={t.artistPlaceholder} /></label>
          <label><span>{t.copyright}</span><input value={textEdits.copyright} onChange={(event) => assignText("copyright", event.target.value)} placeholder={t.copyrightPlaceholder} /></label>
          <label className="wide"><span>{t.keywords}</span><input value={textEdits.keywords} onChange={(event) => assignText("keywords", event.target.value)} placeholder={t.keywordsPlaceholder} /></label>
          <label><span>{t.city}</span><input value={textEdits.city} onChange={(event) => assignText("city", event.target.value)} placeholder={t.cityPlaceholder} /></label>
          <label><span>{t.country}</span><input value={textEdits.country} onChange={(event) => assignText("country", event.target.value)} placeholder={t.countryPlaceholder} /></label>
        </div>
      </section>}
      <div className="batch-actions">
        <button type="button" className="secondary-button" disabled={running || (operation === "metadata" && !metadataReady)} onClick={() => inputRef.current?.click()}><ImagePlus size={16} />{t.choose}</button>
        <button type="button" className="secondary-button" disabled={running || (operation === "metadata" && !metadataReady)} onClick={() => folderRef.current?.click()}><FolderOpen size={16} />{t.folder}</button>
      </div>
