...
已支持对最多 100 张、总计不超过 1 GB 的图片进行**逐张串行**隐私清理或 GPS 删除。每一项都会独立读取、写入、重新读取元数据并核验图像数据；失败或核验未通过的项目不会影响其他项目，且不会提供下载。队列保留逐项状态、失败原因和“仅重试失败项”入口；已验证副本可逐张下载。Chromium 浏览器还可选择文件夹，其他浏览器可直接多选文件。
